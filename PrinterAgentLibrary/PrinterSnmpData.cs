...
namespace PrinterAgentLibrary
{
    public class PrinterSnmpData : IPrinterSnmpData
    {
        ...
        public async Task<PrinterModel> GetPrinterData(string ipAddress, string snmpCommunity = "public")
        {

            PrinterModel printer = new PrinterModel();
            printer.IpAddress = ipAddress;

            printer.UpTime = await UpTimeAsync(ipAddress, snmpCommunity);
            ...
            printer.Manufacturer = await Manufacturer(ipAddress, snmpCommunity);
            printer.Model = await Model(ipAddress, snmpCommunity);
            printer.SerialNumber = await SerialNumber(ipAddress, snmpCommunity);
            printer.LifeCount = await PageLifeCount(ipAddress, snmpCommunity);
            printer.CountSinceReboot = await PageCountSinceReboot(ipAddress, snmpCommunity);
            printer.Color = await Color(ipAddress, snmpCommunity);
            printer.Status = await DeviceStatus(ipAddress, snmpCommunity);

            printer.Toner1ColorInPercentage = await TonerColorInPercentage(ipAddress, snmpCommunity, toner1oidArray);
            printer.Toner2ColorInPercentage = await TonerColorInPercentage(ipAddress, snmpCommunity, toner2oidArray);
            printer.Toner3ColorInPercentage = await TonerColorInPercentage(ipAddress, snmpCommunity, toner3oidArray);
            printer.Toner4ColorInPercentage = await TonerColorInPercentage(ipAddress, snmpCommunity, toner4oidArray);

            printer.Tray1InPercentage = await TrayInPercentage(ipAddress, snmpCommunity, paperTray1oidArray);
            printer.Tray2InPercentage = await TrayInPercentage(ipAddress, snmpCommunity, paperTray2oidArray);
            printer.Tray3InPercentage = await TrayInPercentage(ipAddress, snmpCommunity, paperTray3oidArray);
            printer.Tray4InPercentage = await TrayInPercentage(ipAddress, snmpCommunity, paperTray4oidArray);
        ...
        private async Task<string> UpTimeAsync(string ipAddress, string snmpCommunity)
        {
            ...
            Dictionary<Oid, AsnType> result = await snmpGetRequest(ipAddress, snmpCommunity, oidArray);
            ...
            if (result != null)
            {
                ...
                upTime = await GetDesiredUpTimeFormat(ipAddress, snmpCommunity, upTimeArray);
            }
            else
            {
                //upTime = "Offline";
        ...
        private async Task<string> Manufacturer(string ipAddress, string snmpCommunity)
        {
            ...
            if (result != null)
            {
                ...
                manufacturer = await GetManufacturer(resultArray);
            }
            else
            {
        ...
        private async Task<string> Model(string ipAddress, string snmpCommunity)
        {
            // hrDeviceDescr
            ...
            if (result != null)
            {
                model = await GetModelName(ipAddress, snmpCommunity, result);
            }
            else
            {
                //model = "Unknown";
        ...
        private async Task<string> SerialNumber(string ipAddress, string snmpCommunity)
        {
        ...
        private async Task<string> PageLifeCount(string ipAddress, string snmpCommunity)
        {
        ...
        private async Task<string> PageCountSinceReboot(string ipAddress, string snmpCommunity)
        {
        ...
        private async Task<string> Color(string ipAddress, string snmpCommunity)
        {
            ...
            var checkForSecondColor = await snmpGetRequest(ipAddress, snmpCommunity, oidArray);
        ...
        // === TO DO: ===
        private async Task<string> ServiceCode(string ipAddress, string snmpCommunity)
        {
        ...
        // === TO DO: ===
        private async Task<string> DeviceStatus(string ipAddress, string snmpCommunity)
        {
            ...
            if (result != null)
            {
                ...
                deviceStatus = await DetermineDeviceStatus(returnCode);
            }
            else
            {
        ...
        private async Task<TonerColorModel> TonerColorInPercentage(string ipAddress, string snmpCommunity, string[] tonerOidArray)
        {
            TonerColorModel toner = new TonerColorModel();

            Dictionary<Oid, AsnType> result = await snmpGetRequest(ipAddress, snmpCommunity, tonerOidArray);

            if (result != null)
            {
                // result[0] = TonerName 
                string tonerColorToBeChecked = result.Values.ElementAt(0).ToString();
                toner.Name = await GetTonerColor(tonerColorToBeChecked);
                ...
                toner.CurrentLevelInPercentage = await CountLevelInPercentage(maxCapacity, currentLevel);
            }
            else
            {
                toner.Name = "";
        ...
        private async Task<PaperTrayModel> TrayInPercentage(string ipAddress, string snmpCommunity, string[] oidArray)
        {
            ...
            if (result != null)
            {
                ...
                paperTray.CurrentLevelInPercentage = await CountLevelInPercentage(maxCapacity, currentLevel);
            }
            else
            {
                paperTray.Name = "";
        ...
        private async Task<string> GetManufacturer(string[] resultArray)
        {
            return resultArray.ElementAt(0);
        }

        // Method that determine toner color and return proper name of it.
        private async Task<string> GetTonerColor(string tonerColorToBeChecked)
        {
        ...
        private async Task<int> CountLevelInPercentage(double maxCapacity, double currentLevel)
        {
            double temp = 0.0;
            int output = 0;
        ...
        private async Task<string> GetDesiredUpTimeFormat(string ipAddress, string snmpCommunity, string[] upTimeArray)
        {
            //string[] upTimeArray = result.Values.ElementAt(0).ToString().Split( );
            ...
            if (upTimeFormatContainLetter == true)
            {
                ...
                upTime = await RemoveLeadingZeroes(upTimeArray);
            }
            else
            {
                upTime = await GetShortTimeFormat(upTimeArray);
            }
            return upTime;
        }

        // This method belongs "ReturnDesiredUpTimeFormat"
        private async Task<string> GetShortTimeFormat(string[] upTimeArray)
        {
            string upTime = "";
        ...
        private async Task<string> RemoveLeadingZeroes(string[] upTimeArray)
        {
            string output = "";
        ...
        private async Task<Dictionary<Oid, AsnType>> snmpGetRequest(string ipAddress, string snmpCommunity, string[] oidArray)
        {
        ...
        private async Task<string> GetModelName(string ipAddress, string snmpCommunity, Dictionary<Oid, AsnType> result)
        {
            string output = "";
            int counter = 0;
            ...
            string manufacturer = await Manufacturer(ipAddress, snmpCommunity);
        ...
        // TO DO:
        private async Task<string> DetermineDeviceStatus(string result)
        {
            /*
